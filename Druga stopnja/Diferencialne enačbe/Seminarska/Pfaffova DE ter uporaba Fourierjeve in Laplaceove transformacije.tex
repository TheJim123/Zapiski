	...
	\section{Laplaceova transformacija}
		...
		\subsection{Osnovne definicije in rezultati}
			...
			\begin{definicija}
				...
				Naj bo $a\geq 0$. Za funkcijo $\map{f}{[a, \infty)}{\R}$, za katero obstajata taka $k\in \R$ in $M > 0$, da je $\abs{f(t)}\leq Me^{kt}~\forall t\in [a, \infty)$, pravimo, da je \pojem{funkcija eksponentnega naraščanja} za $M$ in $k$ na $[a,\infty)$.
			\end{definicija}
			...
			Dopolnimo definicijo z zadostnim pogojem, kdaj bo transformiranka obstajala:
