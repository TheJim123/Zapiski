	...
	\section{Laplaceova transformacija}
		...
		\subsection{Osnovne definicije in rezultati}
			...
			\begin{opomba}
				...
				Od zdaj naprej razširimo vsako funkcijo $\map{f}{[0,\infty)}{\C}$ na $\R$ tako, da ji v negativnih $t$ določimo vrednost $0$.
			\end{opomba}
			\begin{trd}
			...
			\begin{proof}
				...
				Zadnja enakost velja, ker je funkcija $f_a(t)$, kot že vemo, absolutno integrabilna na $\R$. Opazimo tudi, da funkcija $f_a$ zadošča pogojem za uporabo inverzne Fourierjeve transformacije. Velja torej: \begin{align*}
					...
					&= \lim_{R\to\infty}\frac{1}{2\pi}\underbrace{\int_{-R}^{R}F(a+iu)e^{itu}du}_{z = a+iu} \\
