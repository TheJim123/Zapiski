...
\begin{document}
	...
	\section{Fourierjeva transformacija}
		...
		\begin{zgled}
			...
			Denimo sedaj, da $y(x)$ zadošča pogojem, da obstaja inverzna Fourierjeva transformacija. Izkaže se, da je njen izračun malce zapleten. Rešitev se izkaže, da je $y(x) = C\delta(x) - i\delta'(x) - \frac{i}{3}\delta^{(3)}(x)$, kjer je $\delta(x)$ t.~i.~ \pojem{Diracova delta funkcija}.
		\end{zgled}
		
		Ta zgled nam služi v poduk: S Fourierjevo transformacijo si poenostavimo en del problema, lahko pa si s tem močno zakompliciramo drugi del - povratek preko inverzne transformacije.
