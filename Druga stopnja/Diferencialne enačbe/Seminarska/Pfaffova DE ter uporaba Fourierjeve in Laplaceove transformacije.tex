	...
	\section{Fourierjeva transformacija}
		...
		\subsection{Osnovne definicije in rezultati}
			...
			\begin{lem}[Riemman-Lebesguova lema]
				\label{lem:R-L}
				Naj bo $f$ odsekoma zvezna $L^1$ funkcija na nekem končnem ali neskončnem intervalu $I$ Potem velja: $\lim_{\lambda\to\infty}\int_I f(t)cos(\lambda t)dt = 0$, $\lim_{\lambda\to\infty}\int_I f(t)sin(\lambda t)dt = 0$ in $\lim_{\lambda\to\infty}\int_I f(t)e^{i\lambda t}dt = 0$
			\end{lem}
			...
			\end{izr}
			Sedaj se pa lotimo lastnosti Fourierjeve transformacije.
			\begin{trd}
			...
			\begin{lem}
				\label{lem:limfx}
				Naj bo $\map{f}{[0, \infty)}{\R}$ odvedljiva povsod, razen morda v končno mnogo točkah in naj bo njen odvod, $f'$, integrabilna funkcija. Dodatno naj velja $\int_0^\infty f(x)dx < \infty$ in $\int_0^\infty f'(x)dx < \infty$. Predpostavimo še, da je $\forall x\in [o,\infty) f(x) = \int_{0}^{x} f'(t)dt + f(0)$. Tedaj je $\lim_{x\to\infty}f(x) = 0$
			\end{lem}
			\begin{proof}
			...
			\begin{izr}
				...
				Naj bo $f$ poljubna absolutno integrabilna funkcija na $\R$ z lastnostjo, da njen odvod, $f'$, obstaja povsod, razen morda v končno mnogo točkah, ter je tudi sama absolutno integrabilna funkcija na $\R$. Denimo še, da je $\forall x\in \R f(x) = \int_0^x f'(t)dt + f(0)$. Tedaj je $\mathcal{F}(f')(y) = (-iy)\mathcal{F}(f)(y)$. Dodatno, denimo, da je $f$ $k$-krat odvedljiva na $\R$ ter, da so $f$ in vsi njeni odvodi absolutno integrabilni na $\R$ ter da je vsak od teh (razen $k$-tega odvoda) integral svojega odvoda. Potem je $\mathcal{F}(f^{(k)})(y) = (-iy)^k\mathcal{F}(f)(y)$.
			\end{izr}
			\begin{proof}
				Trditev bomo pokazali samo za prvi odvod, saj gre pri višjih odvodih le za zaporedno uporabo tega rezultata. Denimo torej, da je $f, f'\in L^1(\R)$, kjer $f'$ obstaja povsod na $\R$, razen morda v končno točkah in naj velja $\forall x\in \R f(x) = \int_0^x f'(t)dt + f(0)$. Potem je: \begin{align*}
			...
			\begin{proof}
				...
				\end{align*}
				Sedaj opazimo, da je $\lim_{h\to 0}\frac{e^{ith}-1}{h} = \frac{d}{dx}(e^{itx})(x=0) = ite^{0} = it$. To pomeni, da je $\lim_{h\to 0}f(t)\frac{e^{ith}-1}{h} = itf(t)$ in ker je po predpostavki $tf(t)$ absolutno integrabilna, integral $\int_{-\infty}^\infty (\lim_{h\to 0}f(t)\frac{(e^{ith}-1)}{h})dt$ obstaja in posledično obstaja tudi integral $\int_{-\infty}^\infty (\lim_{h\to 0}f(t)\frac{(e^{ith}-1)}{h})e^{ity}dt$, ki je po eni strani enak $\int_{-\infty}^\infty itf(t)e^{ity}dt = \mathcal{F}(itf(t))(y)$, po drugi strani pa lahko limito in integral zaradi absolutne integrabilnosti zamenjamo in dobimo ravno $(*)$. Po tem sledi, da je $\frac{d}{dy}\mathcal{F}(f)(y) = \mathcal{F}(itf)(y)$.
			\end{proof}
			
			\begin{posl}
				\label{posl:nodvFT}
				Naj bodo funkcije $f, tf, t^2f, \ldots, t^nf$ vse absolutno integrabilne na $\R$. Tedaj je $\frac{d^n}{dy^n}\mathcal{F}(f(t))(y) = \mathcal{F}((it)^nf(t))(y) \forall y\in \R$.
			\end{posl}
			\begin{proof}
