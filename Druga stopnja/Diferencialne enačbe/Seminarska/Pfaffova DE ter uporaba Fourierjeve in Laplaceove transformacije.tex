...
\begin{document}
	...
	\section{Fourierjeva transformacija}
		...
		\subsection{Osnovne definicije in rezultati}
			...
			\begin{zgled}
				\label{zgl:Fourier1} 
				Določimo Fourierjevo transformiranko funkcije $\chi_{[-1, 1]}(t) = \begin{cases}
					1~&;~ t\in [-1, 1] \\
					0~&;~\text{sicer}
				\end{cases}$.
				
				\begin{align*}
					\mathcal{F}(\chi_{[-1, 1]})(y) &= \int_{-\infty}^{\infty}\chi_{[-1, 1]}(t)e^{ity}dt = \int_{-1}^{1}e^{ity}dt = \frac{1}{iy}e^{ity}\big|_{-1}^{1}\\
					&= \frac{-i}{y}(\cos(ty) + i\sin(ty))\big|_{-1}^{1} = \frac{1}{y}(\sin(ty)-i\cos(ty))\big|_{-1}^{1} \\
					&= \frac{2\sin(y)}{y}
				\end{align*}
			\end{zgled}
			...
			\begin{zgled}
				Funkcija $\chi_{[-1, 1]}$ je dvakrat zvezno odvedljiva in absolutno integrabilna na $\R$. Njena odvoda sta oba ničelni funkciji, torej tudi absolutno integrabilna na $\R$. Potem lahko s pomočjo Fourierjeve transformacije in Parsevalove enakosti določimo $\int_{-\infty}^{\infty}\frac{\sin^2(y)}{y^2}dy$. Po zgledu \ref{zgl:Fourier1} namreč vemo, da je $\mathcal{F}(\chi_{[-1, 1]})(y) = \frac{2\sin(y)}{y}$. Ko upoštevamo Parsevalovo enakost, na desni strani dobimo $4\int_{-\infty}^{\infty}\frac{\sin^2(y)}{y^2}dy$, na levi pa $\int_{-1}^{1}1^2dt = 2$. Sledi, da je $\int_{-\infty}^{\infty}\frac{\sin^2(y)}{y^2}dy = \frac{1}{2}$.
			\end{zgled}
